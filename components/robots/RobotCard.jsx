...
function RobotCard({name, avatar, manufacturerName, robotUsername, birthCountry, birthCity, birthState, birthDate, price, available, type, manufacturerEmail, manufacturerPhone, manufacturerWebsite, ...props}) {
    ...
    return ( 
        <article className="w-full sm:w-[calc(50%-1rem)] lg:w-[calc(33%-1.75rem)] bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
            <header>
                <Image 
                    src={avatar}
            ...
            <dl className="p-5">
                ...
                <dd className="font-normal text-gray-700 dark:text-gray-400">{manufacturerWebsite}</dd>
                ...
                <dd className={`${available ? variants.available : variants.unavailable} mx-auto w-fit items-center px-3 py-2 text-sm font-medium text-center rounded-lg`}>
